...
const Decklist = ({
  ...
  <Column lg="50%" md="100%">
    ...
    <List id="decklist">
      {deck.cards.map(card => (
        <DeckListCard
          ...
          showCardClass
          boxShadowColor={
            isOrigin(deck, card) ? "#E48B19" : isOther(deck, card) && "#006AFF"
          }
        />
      ))}
    </List>
    ...
    <Paragraph>
      ...
      The cards in <b>orange name</b> (if any) are completely random good cards.
      All the others are selected by the algorithm.
    </Paragraph>
    <Paragraph>
      <span role="img" aria-label="bullet point emoji">
        &#x2728;
      </span>
      The cards in <b>orange border</b> are either the starting cards you select
      or those the algorithm selects.
    </Paragraph>
    <Paragraph>
      <span role="img" aria-label="bullet point emoji">
        &#x2728;
    ...
    <Paragraph>
      ...
      <a
        href="#openhistory"
        onClick={e => e.preventDefault() || handleOpenHistoryModal()}
      >
        {" "}
        History section{" "}
      </a>
