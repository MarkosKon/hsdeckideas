...
import { fieldEquals, fieldGreaterThan, fieldLessThan } from "./object";
...
export const message = {
  ...
  notAddingWeAlreadyHave: cardCount =>
    `We decided against adding extra cards because we already have in the deck
    ${cardCount} cards that meet those requirements.`,
  successfullyAdded: (cardsAdded, availableCards, deckCards) =>
...
export const getDeck = (
  ...
  if (extraDeckWideFilters.length > 0)
    availableCards = getCardsForFilters(
      availableCards,
      extraDeckWideFilters,
      true
    );
  ...
  addInterestingCards(
    ...
    interestingCards
      ? interestingCards
      : [chooseInterestingCard(availableCards, deck.cards)]
  );

  let currentStep = getLastStep(deck);

  // other cards.
  currentStep.otherCards = otherCards
    ? addOtherCards(deck, currentStep, otherCards)
    : [];
  ...
  if (deck.archetype !== "Random") {
    currentStep.priorities = currentStep.priorities.concat(
      deck.archetype.priorities
    );
  ...
  while (sizeLessThan30(deck) && continueLooping && numberOfSteps < 20) {
    ...
    if (firstTime) firstTime = false;
      ...
      // Priorities.
      currentStep.priorities = obtainPriorities(deck);
      currentStep.prioritiesInfo = getInfoFromPriorities(
    ...
    if (currentStep.deckWideFilters.length > 0) {
      availableCards = getCardsForFilters(
        availableCards,
        currentStep.deckWideFilters,
        false
      ); // A.
      const remainingDeckCards = getCardsForFilters(
        deck.cards,
        currentStep.deckWideFilters,
        false
      );
    ...
    if (sizeLessThan11(deck) && currentStep.priorities.length === 0) {
      ...
      anInterestingCard.quantity = calculateCardQuantity(
        deck,
        anInterestingCard
      );
    ...
    if (sizeLessThan30(deck))
      deck.history.steps.push(
        initializeStep(currentStep.totalAddedCards, deck)
      );
...
export const hasDuplicates = deck =>
  deck.cards
    .sort(byName)
    .reduce(
      (result, card, i, cards) =>
...
export const completeDeckByPriorities = (deck, availableCards, priorities) => {
  ...
  for (let priority of priorities) {
    let currentPriorityInfo = currentStep.prioritiesInfo.find(
...
export const addCardsForPriority = (
  ...
  availableCardsThatSatisfyPriority = removeSubset(
    availableCardsThatSatisfyPriority,
    deck.cards
  );
  ...
  while (
    numberOfCardsWeWantToAdd > 0 &&
    availableCardsThatSatisfyPriority.length > 0
  ) {
    let cardToPut = getCard(
      availableCardsThatSatisfyPriority,
      deck.isCompetitive
    );
    removeArrayElement(availableCardsThatSatisfyPriority, cardToPut);
    cardToPut.quantity = calculateCardQuantity(
      deck,
      cardToPut,
      numberOfCardsWeWantToAdd < 2
    );
  ...
  if (cardsWeAdded.length === 0)
    currentPriorityInfo.extra = message.notAddingWeAlreadyHave(
      totalDeckCardsThatSatisfyPriority
    );
  else
  ...
  currentStep.totalAddedCards = currentStep.totalAddedCards.concat(
    cardsWeAdded
  );
...
export const calculateHowManyCardsToPut = (
  ...
  let minNumberOfCardsToPut =
    priority.minCards - totalDeckCardsThatSatisfyPriority;
  if (minNumberOfCardsToPut < 0) minNumberOfCardsToPut = 0;

  const maxNumberOfCardsToPut =
    priority.maxCards - totalDeckCardsThatSatisfyPriority;
  ...
  if (freeSlots < minNumberOfCardsToPut) numberOfCardsToPut = freeSlots;
    ...
    numberOfCardsToPut = getRandom(
      minNumberOfCardsToPut,
      maxNumberOfCardsToPut
    );
...
export const calculateCardQuantity = (deck, card, otherCase) =>
  otherCase !== undefined
    ? isHighlander(deck) || sizeEquals29(deck) || isLegendary(card) || otherCase
      ? 1
      : 2
...
export const getClosestArchetype = (deck, archetypes) => {
  ...
  const mostSatisfiedArchetypes = archetypeStats.filter(
    stat => stat.cardCount === max
  );
  if (mostSatisfiedArchetypes.length > 1)
    return mostSatisfiedArchetypes[
      getRandom(0, mostSatisfiedArchetypes.length - 1)
    ].archetype;
...
export const deckSatisfiesPriority = (deck, priority) => {
  let cardCount = getSize(
    getCardsForFilters(deck.cards, priority.filters, false)
  );
  ...
  deck.cards.find(
    card =>
      ...
      card.versions
        ...
        .find(p => idEquals(p, priorityId))
  );

export const versionsToPriorities = (priorities, version) =>
  priorities.concat(version.priorities);
 ...
 * @param {Object} deck
 */
export const obtainPriorities = deck =>
  deck.cards
    .filter(hasPriorities)
    .reduce(toPriorities, [])
    .filter(priority => !isPriorityExamined(deck, priority));

const toPriorities = (priorities, card) =>
  priorities.concat(getActiveVersion(card).priorities);

const isPriorityExamined = (deck, priority) =>
  deck.history.totalPrioritiesExamined[priority.id];
...
export const obtainDeckWideFilters = deck =>
  deck.cards
