...
it("getDeck test #1: Checks if returns a deck with 30 cards for Standard Druid.", () => {
  ...
  let availableCards = getAvailableCards(cards, heroName, format);
...
it(`getDeck test #9: Checks if returns a deck with 30 cards if we have 1 extra deck wide filter`, () => {
  ...
  const format = "Standard";
  const availableCards = getAvailableCards(cards, heroName, format);
  const extraDeckWideFilters = extraFilters.find(
    filter => filter.name === "Curse of Naxxramas"
  );
  let deck = initializeDeck({
    heroName: heroName,
    heroPower: heroPower,
...
it(`getDeck test #10: Checks if returns a deck with 30 cards if we have multiple extra deck wide filter`, () => {
  ...
  const extraDeckWideFilters = extraFilters.filter(filter =>
    expansionNames.includes(filter.name)
  );
  let deck = initializeDeck({
    heroName: heroName,
    heroPower: heroPower,
...
it("toCardCount test #4: Checks if takes into account the hero power for +2 cards.", () => {
  const deckCards = cards
    .filter(c =>
      ["Leper Gnome", "Headcrack", "Captain Greenskin"].includes(c.name)
    )
    .map(c => {
      c.quantity = 2;
      return c;
...
it("deckSatisfiesPriority test #1: Checks if returns the count of cards in deck that satisfy the priority.", () => {
  ...
  const result = deckSatisfiesPriority(
    deck,
    knifeJuggler.versions[0].priorities[0]
  );
  expect(result).toEqual(4);
});
...
it("obtainPriorities test #3: Checks if extracts the priorities of a deck with 2 cards.", () => {
  ...
  const result = obtainPriorities(deck);
  expect(result).toEqual(
    soulOfTheForest.versions[0].priorities.concat(
      jungleGiants.versions[0].priorities
    )
  );
});
...
it("obtainPriorities test #4: Checks if extracts the priorities of a deck with 2 cards and 1 of them already examined.", () => {
  ...
  const deck = {
    ...
    history: {
      totalPrioritiesExamined: {
        "25f04c7a-5598-4a32-a1ff-83e13939432b":
          soulOfTheForest.versions[0].priorities[0],
        "d929cc68-2c90-4810-8728-8eead6290f4d":
          soulOfTheForest.versions[0].priorities[1]
      }
    }
  };
  const result = obtainPriorities(deck);
  ...
  const deck = {
    ...
    history: {
      totalPrioritiesExamined: {
        ...
        "d929cc68-2c90-4810-8728-8eead6290f4d":
          soulOfTheForest.versions[0].priorities[1],
        "9510e0a3-8645-4c9d-bb13-da9c023b8860":
          jungleGiants.versions[0].priorities[0],
        "84bff91e-644b-41e7-8012-28e51e17c1a2":
          jungleGiants.versions[0].priorities[1],
        "c246a843-a6e3-472f-82af-879360e09e83":
          jungleGiants.versions[0].priorities[2]
      }
    }
  };
  const result = obtainPriorities(deck);
...
it("getInfoFromPriorities test #2: Checks if gets info from priorities correctly", () => {
  const priorities = findCardByName(cards, "Effigy").versions.reduce(
    versionsToPriorities,
    []
  );
