...
export const getCard = (cards, isCompetitive) =>
  isCompetitive ? getBestCard(cards) : getRandomCard(cards);
...
export const isCardInteresting = card =>
  hasPriorities(card) || hasDeckWideFilters(card);
...
export const cardExists = (cards, card) =>
  cards.find(c => c.name === card.name);

export const findCardByName = (cards, name) => cards.find(c => c.name === name);
export const findCardsByNames = (cards, names) =>
  cards.filter(c => names.includes(c.name));

export const removeSubset = (array, subset) =>
  array.reduce(
    (result, item) =>
...
//  by hero and format.
export const getCardsForFilters = (
  filteredCards,
  filters,
  breakIfOneFilterIsTrue
) =>
...
export const cardSatisfiesFilters = (card, filters, breakIfOneFilterIsTrue) => {
  ...
  for (const filter of filters) {
    ...
    if (
      get(card, filter.property) === undefined &&
      filter.operation !== "IS_UNDEFINED"
    ) {
      satisfiesFilters = false;
      break;
    }
