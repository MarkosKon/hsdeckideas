...
const cardCountWPrioritiesThatFindNothing = (availableCards, interestingCards, boundary, secret) =>
    interestingCards
        ...
        .reduce((shitCardCount, priority) => 
            getCardsForFilters(availableCards, priority.filters, false).length === boundary
                ? ++shitCardCount
                : shitCardCount
        , 0);
    ...
    return interestingCards
            ...
            .reduce((shitCardCount, priority) => {
                let maxPossibleNumber 
                    = getCardsForFilters(availableCards, priority.filters, false)
...
it("Data test #24: Checks if the unique property number of the card object is 39.", () => {
    ...
    const expectedResult = [
        "hideStats", "armor", "faction",
        "entourage", "classes", "multiClassGroup",
        "collectionText", "deckFilters", "howToEarn",
        "howToEarnGolden", "spellDamage", "overload",
        "durability", "targetingArrowText", "elite",
        "referencedTags", "attack", "health", "race",
        "mechanics", "activeVersion", "versions",
        "name", "artist", "cardClass",
        "collectible", "cost", "dbfId",
        "flavor", "id", "playRequirements",
        "rarity", "set", "rating", "text",
        "type", "extra", "imageUrl", "tile"];
...
const extractPropertyNames = (uniquePropertyNames, nextObject) => 
    Object.getOwnPropertyNames(nextObject)
...
const extractUniqueArrayItems = (uniqueItems, nextItem) =>
    !uniqueItems.includes(nextItem) ? uniqueItems.concat(nextItem) : uniqueItems;
const extractUniqueObjectProperties = (property, uniqueProperties, nextObject) =>
    !uniqueProperties.includes(nextObject[property])
...
const createArtistStats = (artistStats, nextArtistName) => {
    const indexedArtist = artistStats.find(indexedArtist => indexedArtist.name === nextArtistName)
    if (indexedArtist)
        indexedArtist.workCount++;
    else 
        artistStats.push({name: nextArtistName, workCount: 1})
    return artistStats;
}
...
it("Data test #26: Checks if the are 13 options available for the cardClass property.", () => {
    const expectedResult = [
        "MAGE", "HUNTER", "PRIEST", "NEUTRAL",
        "WARLOCK", "ROGUE", "DRUID", "SHAMAN",
        "WARRIOR", "PALADIN",
...
it("Data test #28: Checks if the are 22 options available for the referencedTags property.", () => {
    const expectedResult = [
        "SPELLPOWER", "CHARGE", "TAUNT", "STEALTH", 
        "IMMUNE", "DIVINE_SHIELD", "SECRET", "WINDFURY", 
        "SILENCE", "BATTLECRY", "OVERLOAD", "FREEZE", 
        "JADE_GOLEM", "DISCOVER", "CHOOSE_ONE", "DEATHRATTLE", 
        "COUNTER", "POISONOUS", "LIFESTEAL", "RECRUIT", 
        "ADAPT", "COMBO", "RUSH", "GEARS", "MODULAR"
    ];
    const result = 
        cards
...
it("Data test #29: Checks if the are 35 options available for the mechanics property.", () => {
    const expectedResult = [
        "SECRET", "RECEIVES_DOUBLE_SPELLDAMAGE_BONUS", "INSPIRE", 
        "BATTLECRY", "DEATHRATTLE", "InvisibleDeathrattle", 
        "COMBO", "CHOOSE_ONE", "AURA", 
        "OVERLOAD", "TAUNT", "CHARGE", 
        "DIVINE_SHIELD", "FORGETFUL", "SPELLPOWER", 
        "STEALTH", "CANT_ATTACK", "TOPDECK", 
        "WINDFURY", "JADE_GOLEM", "LIFESTEAL", 
        "DISCOVER", "FREEZE", "ENRAGED", 
        "ADJACENT_BUFF", "POISONOUS", "SILENCE", 
        "ImmuneToSpellpower", "CANT_BE_TARGETED_BY_SPELLS", "CANT_BE_TARGETED_BY_HERO_POWERS",
        "DEATH_KNIGHT", "RITUAL", "QUEST", 
        "RUSH", "ECHO", "MODULAR", "AFFECTED_BY_SPELL_POWER",
        "TRIGGER_VISUAL", "FINISH_ATTACK_SPELL_ON_DAMAGE"
    ];
    const result = 
        cards
...
it("Data test #30: Checks if the are 10 options available for the race property.", () => {
    const expectedResult = [
        " ", "DRAGON", "DEMON", "PIRATE", "BEAST", 
        "TOTEM", "MURLOC", "ELEMENTAL", "MECHANICAL", "ALL"
    ];
...
it("Data test #38: Checks if the are 15 options available for the extra property.", () => {
    const expectedResult = [
        "HARD_REMOVAL", "SECRET", "SMALL_REMOVAL",
        "FAST", "TEMPO", "GENERAL",
        "VALUE", "MIN_GEN", "DAMAGE",
        "BUFF", "HEAL", "CARD_DRAW",
        "AOE", "STALL", ""
    ];
    const result =
